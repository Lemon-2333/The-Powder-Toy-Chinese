...
ElementSearchActivity::ElementSearchActivity(GameController * gameController, std::vector<Tool*> tools) :
	...
	exit(false)
{
	ui::Label * title = new ui::Label(ui::Point(4, 5), ui::Point(Size.X-8, 15), ByteString("元素搜索").FromUtf8());
	...
	ui::Button * okButton = new ui::Button(ui::Point(Size.X/2, Size.Y-15), ui::Point(Size.X/2, 15), ByteString("确定").FromUtf8());
